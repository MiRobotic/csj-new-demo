...
import com.csjbot.mobileshop.util.RichTextUtil;
import com.csjbot.mobileshop.util.SharedKey;
import com.csjbot.mobileshop.util.SharedPreUtil;
import com.csjbot.mobileshop.util.TonePlayer;
...
public class SettingsVolumeActivity extends BaseModuleActivity implements RangeSeekBar.OnRangeChangedListener {
    ...
    public void afterViewCreated(Bundle savedInstanceState) {
        ...
        // 如果不是日语的小雪机器人，就可以选择
        if (!(CsjLanguage.isJapanese() && BuildConfig.FLAVOR.equals(BuildConfig.ROBOT_TYPE_DEF_SNOW))) {
            goToChooseVoiceSpeaker.setVisibility(View.VISIBLE);
        }

        if (CsjLanguage.isChinese() && !Constants.isOpenNuance) {
        ...
        boolean isChecked = SharedPreUtil.getBoolean(SharedKey.AUTO_SPEECH_RECOGNITION_SWITCH, SharedKey.AUTO_SPEECH_RECOGNITION_SWITCH, true);
    ...
    public int getLayoutId() {
        return getCorrectLayoutId(R.layout.activity_settings_volume, R.layout.vertical_activity_settings_volume);
...
//        unregisterVolumeChangeReceiver();
    ...
    public void openMultilingualRecognition(View view) {
        ...
        com.csjbot.mobileshop.model.http.factory.ServerFactory.createApi().getInternationalStatus(Robot.SN, new Observer<ResponseBody>() {
            @Override
