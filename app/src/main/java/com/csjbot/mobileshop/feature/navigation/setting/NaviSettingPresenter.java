...
import com.csjbot.mobileshop.dialog.MapSelectDialog;
import com.csjbot.mobileshop.feature.navigation.adapter.FileListAdapter;
...
import com.csjbot.mobileshop.widget.MoveView;
...
//        removeMediaRes(navis.get(position));
    ...
    private static final class MyOnPositionListener implements OnPositionListener {
        ...
        public void positionInfo(String json) {
            CsjlogProxy.getInstance().info("NAVI->导航设置 positionInfo"+json);
            if (reference != null && reference.get().view != null && json != null) {
        ...
        public void moveResult(String json) {
            CsjlogProxy.getInstance().info("NAVI->导航设置 moveResult"+json);
        }

        @Override
        public void moveToResult(String json) {
            CsjlogProxy.getInstance().info("NAVI->导航设置 moveToResult"+json);
        }

        @Override
        public void cancelResult(String json) {
            CsjlogProxy.getInstance().info("NAVI->导航设置 cancelResult"+json);
        }
    }
