...
/**
 * @项目名 BlackGaGa
 * @路径 name：com.csjbot.blackgaga.util
 * @创建者 Wql
 * @创建时间 2017/12/1 14:41
 */
public class DownloadUtil implements Serializable {
    ...
    public static boolean isCanceled = false;           // 是否手动停止下载
     ...
     */
    public DownloadUtil(int downloadId, String url, String localPath) {
        if (!new File(localPath).exists()) {
            new File(localPath).mkdirs();
        }

        BlackgagaLogger.debug("下载地址；" + url);
    ...
    public void start(final boolean isGoon) {
        ...
        // 真正开始下载
        mThreadPool.execute(() -> download(isGoon, handler));
    }
        ...
        try {
            ...
            if (httpCode >= 200 && httpCode <= 300) {
                ...
                while (-1 != (len = in.read(bytes))) {
                    ...
                    long progress = (long) (downloadedLength / (float) remoteFileLength * 100);
//                    if ((int) (downloadedLength / (float) remoteFileLength * 100) % 10 == 0) {
                    ...
                    msg.obj = progress;
                    handler.sendMessage(msg);
                    BlackgagaLogger.debug(mDownloadId + "已下载-->" + downloadedLength);
                    BlackgagaLogger.debug(mDownloadId + "已下载progress-->" + progress);
//                    }
                    ...
                    if (isPause) {
                        ...
                        BlackgagaLogger.debug("下载暂停...");
                        break;
                    }

                    // 取消下载， 删除文件并退出方法
                    if (isCanceled) {
                        BlackgagaLogger.debug("手动关闭下载。。");
                        localFile.close();
    ...
    public synchronized boolean pause(boolean pause) {
        if (!pause) {
            BlackgagaLogger.debug("继续下载");
            isPause = false;
            start(true); // 开始下载
        } else {
            BlackgagaLogger.debug("暂停下载");
            isPause = true;
        }
        return isPause;
    }
    ...
    public synchronized long getLocalFileLength() {
        ...
        BlackgagaLogger.debug("本地文件大小" + size);
    ...
    public synchronized long getRemoteFileLength() {
        ...
        try {
            ...
            HttpResponse response = client.execute(get);
            int httpCode = response.getStatusLine().getStatusCode();
            BlackgagaLogger.debug("httpCode" + httpCode);
            if (httpCode >= 200 && httpCode <= 300) {
        ...
        BlackgagaLogger.debug("远程文件大小" + size);
        return size;
    }
