...
public class MapSelectDialog extends Dialog {
    ...
    public void scanFile(File root, Handler handler) {
        ...
        if (files != null && files.length > 0) {
            for (File file : files) {
                ...
                if (fileName.contains(NaviActivity.suffixs[0])
                        || fileName.contains(NaviActivity.suffixs[1])
                        || fileName.contains(NaviActivity.suffixs[2])
                        ) {
    ...
    class MyHandler extends Handler {
        WeakReference<MapSelectDialog> mContextWeakReference = null;
        public MyHandler(MapSelectDialog usbFragment) {
            super();
        ...
        public void handleMessage(Message msg) {
            ...
            MapSelectDialog mapSelectDialog = mContextWeakReference.get();
    ...
    public void setListener(MapSelectDialog.OnDialogClickListener listener){
        mListener = listener;
    }
