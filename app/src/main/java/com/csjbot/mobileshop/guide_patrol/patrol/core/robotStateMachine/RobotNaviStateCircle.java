...
public class RobotNaviStateCircle extends RobotNaviStateBase{

    private static final String TAG = RobotNaviStateCircle.class.getSimpleName();
    private static volatile int curIndex = 0;
    private static List<Position> workList = new ArrayList<>();//一键导航点信息

    public RobotNaviStateCircle(StateMachineManager mgr) {
        super(mgr);
        naviTaskStatus = NaviTaskStatus.CIRCLE;
    }
    ...
    private void nextGuide() {
        Log.e("sunxy","继续导航出错"+curIndex+workList.size());
        if (workList != null && curIndex < workList.size()) {
    ...
    public void moveActionResult(boolean isSuccess) {
        if (!isSuccess) {//失败的话，再次发点
            ...
            curIndex++;
            if (curIndex == getCirclePos().size()) {
                curIndex = 0;
            }
            Log.e("sunxy","moveActionResult"+curIndex);
            NaviTaskUtils.setCurCircleIndex(curIndex);
            NaviTaskUtils.setCircleStatus(1);
